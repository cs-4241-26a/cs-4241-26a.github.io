...
## Using the MongoDB core driver
  ...
  - Run `npm i express mongodb dotenv --save` in your project directory.
  - Create a `.env` file to hold your authentication data. *MAKE SURE TO PUT THE HOST VALUE IN QUOTATION MARKS*:
```
USER=xxxxx
PASS=xxxxx
...
```
  ...
  We'll use the [dotenv library](https://www.npmjs.com/package/dotenv) to read our dotfile.
  - You can get all of the above login information by going to Atlas > Clusters > Connect > Connect Your Application
...
We'll test this using [curl](https://curl.se/). `curl --request GET "127.0.0.1:3000/docs"` in your command line. 
...
We'll again test using curl, but we need to make sure we include
the `Content-Type:application/json` header in the data we pass, otherwise `req.body` won't be populated
by express.

`curl --request POST '127.0.0.1:3000/add' -d '{ "name":"myname" }' -H "Content-Type:application/json"`
